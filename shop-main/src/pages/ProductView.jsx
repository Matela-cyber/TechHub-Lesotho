...
const categoryIcon = (type) => {
  const map = {
    ...
    "Printers & Scanners": "🖨️",
    "Storage Devices": "💾",
    "Accessories & Peripherals": "🖱️",
    "Smartphones & Tablets": "📱",
    "Software & Licenses": "📦",
    "Cloud Services & SaaS": "☁️",
    "Web Hosting & Domains": "🌐",
    "VPS & Dedicated Servers": "🗄️",
    "Managed IT Services": "🛠️",
    "Cybersecurity Services": "🔐",
    "Online Courses & Training": "🎓",
    "API & Developer Tools": "⚙️",
...
function ServiceBadges({ product }) {
  ...
  if (product.trialPeriodDays > 0)
    badges.push({
      ...
      text: `${product.trialPeriodDays}-day free trial`,
      color: "bg-green-50 text-green-700 border-green-200",
    });
  if (product.sla?.uptime)
    badges.push({
      icon: <ShieldCheck className="w-4 h-4" />,
      text: `${product.sla.uptime} uptime SLA`,
      color: "bg-purple-50 text-purple-700 border-purple-200",
    });
  if (product.supportTier && product.supportTier !== "community")
    badges.push({
      icon: <Users className="w-4 h-4" />,
      text: `${product.supportTier.replace("-", " ")} support`,
  ...
  if (product.renewalType)
    badges.push({
      ...
      text:
        product.renewalType === "auto"
          ? "Auto-renewal"
          : product.renewalType === "manual"
            ? "Manual renewal"
            : "One-time purchase",
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {badges.map((b, i) => (
        <span
          key={i}
          className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full border ${b.color}`}
        >
          {b.icon}
          {b.text}
        </span>
...
function FeatureList({ features, isService }) {
  ...
  return (
    <div
      className={`rounded-2xl p-5 border ${isService ? "bg-purple-50 border-purple-100" : "bg-blue-50 border-blue-100"}`}
    >
      <h3
        className={`font-bold text-sm uppercase tracking-wider mb-3 ${isService ? "text-purple-700" : "text-blue-700"}`}
      >
      ...
      <ul className="space-y-2">
        {features.map((f, i) => (
          <li key={i} className="flex items-start gap-2 text-sm text-gray-700">
            <CheckCircle
              className={`w-4 h-4 mt-0.5 flex-shrink-0 ${isService ? "text-purple-500" : "text-blue-500"}`}
            />
            {f}
          </li>
        ))}
        ...
        <tbody>
          {specifications.map((spec, i) => (
            <tr key={i} className={i % 2 === 0 ? "bg-gray-50" : "bg-white"}>
              <td className="px-4 py-2.5 font-semibold text-gray-700 w-1/3">
                {spec.key}
              </td>
...
function ServicePurchasePanel({ product, user, dispatch, navigate }) {
  ...
  const handleSubscribe = () => {
    ...
    toast.success(
      "Added to cart! Proceed to checkout to complete your subscription.",
    );
  };
      ...
      <div className="bg-gradient-to-br from-purple-600 to-indigo-700 p-5 text-white">
        <div className="flex items-baseline gap-2">
          ...
          {product.billingCycle && product.billingCycle !== "one-time" && (
            <span className="text-purple-200 text-sm">
              / {billingLabel(product.billingCycle, product.customBillingLabel)}
            </span>
          )}
        </div>
        {disc > 0 && (
        ...
        {product.billingCycle === "monthly" && product.annualPrice && (
          <p className="text-xs text-purple-200 mt-1">
            or {formatPrice(product.annualPrice)}/year (save{" "}
            {Math.round((1 - product.annualPrice / (product.price * 12)) * 100)}
            %)
          </p>
        )}
        ...
        <ul className="space-y-2 text-xs text-gray-600 pt-1 border-t border-gray-100">
          {product.sla?.uptime && (
            <li className="flex items-center gap-2">
              ...
              {product.sla.uptime} uptime SLA
            </li>
          )}
          {product.supportTier && (
            <li className="flex items-center gap-2">
              <Users className="w-3.5 h-3.5 text-purple-500" />{" "}
              {product.supportTier.replace(/-/g, " ")} support
            </li>
          )}
          {product.maxUsers > 0 && (
            <li className="flex items-center gap-2">
              <Users className="w-3.5 h-3.5 text-purple-500" /> Up to{" "}
              {product.maxUsers} users
            </li>
          )}
          {product.maxUsers === 0 && product.licenseType && (
            <li className="flex items-center gap-2">
              <Users className="w-3.5 h-3.5 text-purple-500" />{" "}
              {product.licenseType.replace(/-/g, " ")} license
            </li>
          )}
          {product.apiKeyProvided && (
            <li className="flex items-center gap-2">
              <Key className="w-3.5 h-3.5 text-purple-500" /> API key / licence
              key included
            </li>
          )}
          {product.renewalType && (
          ...
          {product.certificateIncluded && (
            <li className="flex items-center gap-2">
              <Award className="w-3.5 h-3.5 text-amber-500" /> Certificate of
              completion included
            </li>
          )}
          ...
          {product.contractLength && (
            <li className="flex items-center gap-2">
              <Clock className="w-3.5 h-3.5 text-blue-500" /> Contract:{" "}
              {product.contractLength}
            </li>
          )}
          {product.dataQuota && (
            <li className="flex items-center gap-2">
              <Wifi className="w-3.5 h-3.5 text-blue-500" /> Quota:{" "}
              {product.dataQuota}
            </li>
          )}
        </ul>
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden sticky top-4 space-y-0">
      ...
      <div className="p-5 space-y-4">
        ...
        >
          <div
            className={`w-2 h-2 rounded-full ${inStock ? "bg-green-500 animate-pulse" : "bg-red-500"}`}
          />
          {inStock
        ...
        <ul className="space-y-2 text-xs text-gray-600 pt-1 border-t border-gray-100">
          <li className="flex items-center gap-2">
            <Truck className="w-3.5 h-3.5 text-blue-500" /> Free delivery on
            orders over M500
          </li>
          <li className="flex items-center gap-2">
          ...
          {product.warranty?.available && (
            <li className="flex items-center gap-2">
              ...
              {product.warranty.period} warranty included
            </li>
          )}
          {product.importDetails?.isImported && (
            <li className="flex items-center gap-2">
              <Globe className="w-3.5 h-3.5 text-purple-500" /> Imported from{" "}
              {product.importDetails.country}
            </li>
          )}
          {product.origin && (
            <li className="flex items-center gap-2">
              <Globe className="w-3.5 h-3.5 text-purple-500" /> Made in{" "}
              {product.origin}
            </li>
          )}
        </ul>
        ...
        <div className="container mx-auto px-4 py-8 max-w-7xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-10 mb-12">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              >
                <div
                  className={`rounded-2xl overflow-hidden shadow-lg border aspect-square ${
                    isService
                    ...
                    {[product.image2, product.image3]
                      ...
                      .map((img, i) => (
                        <img
                          ...
                          src={img}
                          alt=""
                          className="w-20 h-20 object-cover rounded-lg border-2 border-gray-200 cursor-pointer hover:border-purple-400 transition-all"
                        />
                      ))}
                  </div>
                )}
                ...
                >
                  ...
                  {product.accessInstructions && (
                    <div className="bg-gray-900 text-green-400 rounded-2xl p-5 font-mono text-sm">
                      <p className="text-gray-400 text-xs uppercase tracking-widest mb-3 font-sans font-semibold">
                        📋 Getting Started
                      </p>
                    ...
                    <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl p-4">
                      ...
                      <div>
                        <p className="font-bold text-amber-800 text-sm">
                          API Key / Licence Key Included
                        </p>
                        <p className="text-amber-700 text-xs mt-0.5">
                          Your unique key will be delivered to your email
                          {product.credentialsDelivery === "dashboard"
                            ? " and shown in your account dashboard"
                            : product.credentialsDelivery === "both"
                              ? " and shown in your account dashboard"
                              : ""}{" "}
                          after purchase.
                        </p>
                      ...
                      {product.courseFormat && (
                        <div className="bg-white border border-gray-200 rounded-xl p-4 text-center">
                          ...
                          <p className="text-sm font-bold text-gray-800 capitalize">
                            {product.courseFormat.replace(/-/g, " ")}
                          </p>
                        </div>
                      )}
                      {product.courseDuration && (
                          ...
                          <div className="flex items-start gap-3 mt-3">
                            ...
                            <div>
                              <p className="font-semibold text-gray-900">
                                Imported from {product.importDetails.country}
                              </p>
                      ...
                      {expandedSections.warranty && (
                        <div className="px-5 pb-5 border-t border-gray-100">
                          <div className="flex items-start gap-3 mt-3">
                            ...
                            <div>
                              <p className="font-semibold text-gray-900">
                                {product.warranty.period} Warranty
                              </p>
                              <p className="text-sm text-gray-600 mt-1">
