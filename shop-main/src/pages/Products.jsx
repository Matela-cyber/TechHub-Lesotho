  ...
  const activeFiltersCount = useMemo(() => {
    return Object.values(filters).reduce((count, value) => {
      if (typeof value === "object") {
        return (
          count +
          Object.values(value).filter((v) => v !== "" && v !== false).length
        );
      }
      return (
        count + (value !== "" && value !== false && value.length > 0 ? 1 : 0)
      );
    }, 0);
  }, [filters]);
                  ...
                  {Object.entries(filters).map(([key, value]) => {
                    if (key === "priceRange" && (value.min || value.max)) {
                      return (
                        <div
                          key={key}
                          className="flex items-center gap-2 bg-gray-100 text-gray-900 px-3 py-1.5 rounded-lg text-sm"
                        >
                          <span>
                            Price: {value.min ? `M${value.min}` : "Min"} -{" "}
                            {value.max ? `M${value.max}` : "Max"}
                          </span>
                          <button
                            onClick={() =>
