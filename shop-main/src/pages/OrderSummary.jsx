...
function OrderSummary() {
  ...
  useEffect(() => {
    ...
    const fetchOrderDetails = async () => {
      ...
      if (!hasValidUrlParams()) {
        console.error(
          "OrderSummary: Missing or invalid orderId parameter in URL",
        );
        setError(
          "Invalid order link. Please check your URL or access your order from your account page.",
        );
        setLoading(false);
        return;
      }
      ...
      if (!user) {
        ...
        console.log(
          "OrderSummary: Proceeding without authentication for potential direct checkout flow",
        );
      }

      try {
        console.log(
          `OrderSummary: Fetching order details for orderId: ${orderId}`,
        );
        ...
        if (!orderSnapshot.exists()) {
          console.error(
            `OrderSummary: Order not found in database: ${orderId}`,
          );
          setError(
            "Order not found. This order may not exist or may have been removed.",
          );
          setLoading(false);
          return;
        }
        ...
        if (!canAccessOrder(orderData)) {
          console.error(
            `OrderSummary: Unauthorized access attempt. Order userId: ${orderData.userId}, Current user: ${user.uid}`,
          );
        ...
        if (
          ...
          console.error(
            "OrderSummary: Invalid order data - missing or empty items array",
          );
        ...
        console.log(
          "OrderSummary: Order data processed successfully:",
          completeOrder,
        );
  ...
  const handleDownloadInvoice = async () => {
    ...
    try {
      ...
      toast.error(
        "Failed to download invoice: " + (error.message || "Unknown error"),
      );
    } finally {
                ...
                <div className="flex items-start">
                  ...
                  <div className="ml-3">
                    ...
                    <div className="mt-2 text-sm text-yellow-700">
                      <p>
                        Some order details appear to be missing. This may be due
                        to a processing issue. The order was successfully
                        placed, but you may want to contact support for complete
                        details.
                      </p>
                    </div>
                    <div className="mt-4">
