    ...
    try {
      ...
      if (user) {
        try {
          ...
          const itemsToRemove = cartDetails
          ...
          if (itemsToRemove.length > 0) {
            ...
            toast.info(
              `${itemsToRemove.length} previously purchased item(s) were removed from your cart`,
            );
      ...
      setTimeout(() => {
        dispatch(clearCart());
        navigate(
          `/summary?orderId=${orderResult.orderId}&paymentId=${orderResult.paymentId || "direct"}&emailSent=${orderResult.emailSent || "false"}`,
        );
        setProcessingPayment(false);
      }, 1500);
    } catch (error) {
      ...
      if (result.valid) {
        dispatch(
          applyCoupon({
            ...
            applicableProducts: result.isProductSpecific
              ? result.eligibleProductIds
              : [],
            appliedToCartItems: result.isProductSpecific
              ? result.appliedToCartItems
              : [],
          }),
        );
      ...
      <div className="container mx-auto px-4">
        ...
        >
          <div className="flex items-center justify-between">
            <div
              className={`flex flex-col items-center ${currentStep >= 1 ? "text-gray-900" : "text-gray-400"}`}
            >
              <div
            ...
            <div
              className={`flex-1 h-1 mx-4 transition-all ${currentStep >= 2 ? "bg-gray-900" : "bg-gray-200"}`}
            ></div>

            <div
              className={`flex flex-col items-center ${currentStep >= 2 ? "text-gray-900" : "text-gray-400"}`}
            >
              <div
                className={`w-12 h-12 rounded-full flex items-center justify-center mb-2 transition-all ${
                  currentStep >= 2
            ...
            <div
              className={`flex-1 h-1 mx-4 transition-all ${currentStep >= 3 ? "bg-gray-900" : "bg-gray-200"}`}
            ></div>

            <div
              className={`flex flex-col items-center ${currentStep >= 3 ? "text-gray-900" : "text-gray-400"}`}
            >
              <div
                className={`w-12 h-12 rounded-full flex items-center justify-center mb-2 transition-all ${
                  currentStep >= 3
                        ...
                        <div>
                          ...
                          <p className="text-sm text-green-700">
                            {appliedCoupon.discountType === "percentage"
                              ...
                              : `M${appliedCoupon.discountValue} off`}
                          </p>
                        </div>
                        <button
...
function UnifiedCheckoutWithRecaptcha() {
  return (
    <GoogleReCaptchaProvider
      reCaptchaKey={
        process.env.REACT_APP_RECAPTCHA_SITE_KEY || "6LcXXXXXXXXXXXXXXXXXXXXX"
      }
    >
      <UnifiedCheckout />
    </GoogleReCaptchaProvider>
