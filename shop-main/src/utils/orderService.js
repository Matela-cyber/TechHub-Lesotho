...
const processNewOrder = async (orderData, userData) => {
  console.log(
    "🚀 orderService: Processing new order for user:",
    userData?.email,
  );
  ...
  try {
    ...
    console.log(
      "📋 orderService: Generated Order ID:",
      orderId,
      "Payment ID:",
      paymentId,
    );
    ...
    console.log(
      "💾 orderService: Creating order document in Firestore collections",
    );
      ...
      for (const productRead of productReads) {
        if (productRead.doc.exists()) {
          ...
          if (currentStock < productRead.item.quantity) {
            throw new Error(
              `Insufficient stock for ${productRead.item.name}. Available: ${currentStock}, Requested: ${productRead.item.quantity}`,
            );
          }
        } else {
      ...
      for (const productRead of productReads) {
        if (productRead.doc.exists()) {
          ...
          const newStock = Math.max(
            0,
            currentStock - productRead.item.quantity,
          );
          ...
          console.log(
            `📦 orderService: Updated stock for ${productRead.item.name}: ${currentStock} → ${newStock}`,
          );
        }
      }
    ...
    console.log(
      "✅ orderService: Order created successfully with Global ID:",
      orderResult.globalOrderId,
    );
    ...
    if (featureConfig.email.enabled) {
      console.log(
        "📧 orderService: Email service enabled, dispatching order confirmation",
      );
      ...
      try {
        ...
        console.log(
          "📤 orderService: Sending order confirmation email to:",
          userForEmail.email,
        );
        ...
        if (emailResult.success) {
          console.log(
            "✅ orderService: Order confirmation email sent successfully",
          );
        } else {
          console.error(
            "❌ orderService: Failed to send order confirmation email:",
            emailResult.error,
          );
        }
      } catch (emailError) {
        console.error(
          "❌ orderService: Exception in email sending:",
          emailError,
        );
      ...
      console.log(
        "⚠️ orderService: Email service disabled in configuration, skipping email dispatch",
      );
      emailResult = {
        success: false,
        error: "Email disabled in configuration",
      };
    }
    ...
    if (error.code) {
      ...
      switch (error.code) {
        ...
        case "failed-precondition":
          userFriendlyMessage =
            "Order validation failed. Please refresh and try again.";
          break;
        default:
          userFriendlyMessage =
...
const updateOrderStatus = async (
  ...
  console.log(
    `🔄 orderService: Updating order ${orderId} status to ${newStatus}`,
  );
  ...
  try {
    ...
    if (
      ...
      console.warn(
        `⚠️ orderService: Invalid status transition from ${currentStatus} to ${newStatus}`,
      );
    ...
    switch (newStatus) {
      ...
      case ORDER_STATUSES.CANCELLED:
        ...
        updateData.cancellationReason =
          updateInfo.reason || "No reason provided";
        break;
      ...
      default:
        ...
        console.log(
          `📋 orderService: Status update to ${newStatus} - no additional fields required`,
        );
        break;
    }
    ...
    await runTransaction(db, async (transaction) => {
      ...
      if (
        [ORDER_STATUSES.CANCELLED, ORDER_STATUSES.DECLINED].includes(newStatus)
      ) {
        for (const item of currentOrder.items || []) {
          ...
          if (productDoc.exists()) {
            ...
            console.log(
              `📦 orderService: Restored stock for ${item.name}: ${currentStock} → ${restoredStock}`,
            );
          }
        }
      }
    ...
    if (featureConfig.email.enabled && updateInfo.notifyCustomer !== false) {
      try {
        ...
        console.error(
          "❌ orderService: Failed to send status change notification:",
          emailError,
        );
      }
    }
...
const updateOrderToShipped = async (
  ...
  try {
    ...
    if (order.status !== ORDER_STATUSES.PACKED) {
      throw new Error(
        `Order must be in ${ORDER_STATUSES.PACKED} status to ship. Current status: ${order.status}`,
      );
    }
    ...
    const carrier = Object.values(SHIPPING_CARRIERS).find(
      (c) => c.name.toLowerCase() === shipmentInfo.carrier.toLowerCase(),
    );
    ...
    if (featureConfig.email.enabled) {
      try {
        ...
        console.log(
          "📧 orderService: Order shipped notification email sent successfully",
        );
      } catch (emailError) {
        console.error(
          "❌ orderService: Failed to send shipment notification email:",
          emailError,
        );
      }
    }
    ...
    console.log(
      `✅ orderService: Order ${orderId} successfully updated to shipped status`,
    );
  ...
  try {
    ...
    if (includeUserData && orderData.userId) {
      try {
        ...
        console.warn(
          "⚠️ orderService: Could not fetch user data for order:",
          userError,
        );
      }
    }
...
const getAllOrders = async (filters = {}, pagination = {}) => {
  ...
  try {
    ...
    console.log(
      `✅ orderService: Retrieved ${orders.length} orders (showing ${paginatedOrders.length})`,
    );
    ...
    return {
      ...
      hasMore: pagination.limit
        ? orders.length > pagination.limit * (pagination.page || 1)
        : false,
    };
  } catch (error) {
...
const getOrderAnalytics = async (filters = {}) => {
  ...
  try {
    ...
    const totalRevenue = orders.reduce(
      (sum, order) => sum + (order.financials?.total || order.total || 0),
      0,
    );
...
const sendStatusChangeNotification = async (order, newStatus, updateInfo) => {
  ...
  console.log(
    `📧 orderService: Sending ${newStatus} notification for order ${order.orderId}`,
  );
  ...
  switch (newStatus) {
    ...
    default:
      console.log(
        `📧 orderService: No specific email template for status ${newStatus}`,
      );
  }
};
...
const bulkUpdateOrders = async (orderUpdates, adminUserId = "system") => {
  console.log(
    `🔄 orderService: Performing bulk update on ${orderUpdates.length} orders`,
  );
  ...
  try {
    ...
    console.log(
      `✅ orderService: Bulk update completed for ${orderUpdates.length} orders`,
    );
