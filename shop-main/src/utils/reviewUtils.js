...
const reviewUtils = {
    ...
    try {
      ...
      for (const orderDoc of ordersSnapshot.docs) {
        ...
        if (orderData.items && Array.isArray(orderData.items)) {
          const foundItem = orderData.items.find(
  ...
  async getProductReviews(productId) {
    try {
      ...
      const reviewsSnapshot = await getDocs(reviewsQuery);
      // Sort reviews by createdAt descending in JS
  ...
  async submitReview(reviewData) {
    try {
      ...
      const hasPurchased = await this.hasUserPurchasedAndReceived(
        user.uid,
        productId,
      );
      if (!hasPurchased) {
      ...
      const existingReview = await this.hasUserReviewedProduct(
        user.uid,
        productId,
      );
      if (existingReview) {
