...
class CouponService {
  ...
  static async validateCoupon(code, cartItems = [], cartTotal, userId) {
    try {
      ...
      if (now < startDate) {
        logger.info(`Coupon not yet valid: ${normalizedCode}`, {
          userId,
          startDate,
        });
        return {
      ...
      if (now > endDate) {
        logger.info(`Expired coupon used: ${normalizedCode}`, {
          userId,
          endDate,
        });
        return {
      ...
      if (coupon.maxUses > 0 && coupon.usedCount >= coupon.maxUses) {
        logger.info(`Maximum usage reached for coupon: ${normalizedCode}`, {
          userId,
        });
        return {
          valid: false,
      ...
      if (isProductSpecific) {
        ...
        if (eligibleCartItems.length === 0) {
          logger.info(
            `Coupon not applicable to any items in cart: ${normalizedCode}`,
            { userId },
          );
          return {
            valid: false,
        ...
        if (
          ...
          eligibleItemsTotal < coupon.minOrderAmount
        ) {
          logger.info(
            `Minimum order amount not met for product-specific coupon: ${normalizedCode}`,
            {
              userId,
              eligibleItemsTotal,
          ...
          return {
            valid: false,
            message: `This coupon requires eligible products total of M${coupon.minOrderAmount.toLocaleString()}.`,
            coupon: null,
          };
        }
        ...
        logger.info(
          `Valid product-specific coupon applied: ${normalizedCode}`,
          {
            userId,
            eligibleItemsTotal,
            discountAmount,
            eligibleProducts: eligibleCartItems.length,
          },
        );
      ...
      if (coupon.minOrderAmount > 0 && cartTotal < coupon.minOrderAmount) {
        logger.info(
          `Minimum order amount not met for coupon: ${normalizedCode}`,
          {
            userId,
            cartTotal,
            minRequired: coupon.minOrderAmount,
        ...
        return {
          valid: false,
          message: `This coupon requires a minimum order of M${coupon.minOrderAmount.toLocaleString()}.`,
          coupon: null,
        };
      }
  ...
  static async recordCouponUsage(couponId) {
    try {
      ...
      logger.error(
        `Error recording coupon usage: ${couponId}`,
        error,
        "CouponService",
      );
      return false;
  ...
  static formatDiscount(coupon, discountAmount, isProductSpecific = false) {
    ...
    if (coupon.discountType === "percentage") {
      return `${coupon.discountValue}% OFF${productText} (M${discountAmount.toLocaleString(
        "en-LS",
        {
      ...
      return `M${discountAmount.toLocaleString("en-LS", {
        maximumFractionDigits: 2,
        minimumFractionDigits: 2,
      })} OFF${productText}`;
    }
