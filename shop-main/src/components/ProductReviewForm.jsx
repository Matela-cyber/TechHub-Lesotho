...
function ProductReviewForm({ productId, onReviewSubmitted }) {
  ...
  useEffect(() => {
    const checkEligibility = async () => {
      ...
      try {
        // Check if user has already reviewed this product (no compound where)
        const reviewsQuery = query(
          collection(db, "reviews"),
          where("productId", "==", productId),
        );
        const reviewSnapshot = await getDocs(reviewsQuery);
        if (reviewSnapshot.docs.some((doc) => doc.data().userId === user.uid)) {
          setHasReviewed(true);
        ...
        ordersSnapshot.forEach((doc) => {
          ...
          if (orderData.items && Array.isArray(orderData.items)) {
            ...
            const productFound = orderData.items.some(
        ...
        setErrorMessage(
          "An error occurred while checking if you can review this product.",
        );
      }
    };
        ...
        {[1, 2, 3, 4, 5].map((star) => (
          ...
          >
            <Star
              size={24}
              fill={
                hoveredRating >= star || rating >= star ? "#F59E0B" : "none"
              }
              stroke={
                hoveredRating >= star || rating >= star ? "#F59E0B" : "#D1D5DB"
              }
