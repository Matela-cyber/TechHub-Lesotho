...
export async function onRequest(context) {
  ...
  console.log(
    `[Email Function] Incoming ${request.method} request to ${request.url}`,
  );
  console.log(
    `[Email Function] Request headers:`,
  ...
  try {
    ...
    try {
      ...
      console.log(
        `[Email Function] Raw request body length: ${rawBody.length}`,
      );
      ...
      console.error(
        "[Email Function] Failed to parse request body:",
        parseError,
      );
      return new Response(
        JSON.stringify({
    ...
    if (missingFields.length > 0) {
      console.error(
        `[Email Function] Missing required fields: ${missingFields.join(", ")}`,
      );
      return new Response(
        JSON.stringify({
          error: {
    ...
    console.log(
      ...
      env.RESEND_API_KEY?.startsWith("re_") || false,
    );
    ...
    if (!apiKey) {
      console.error(
        "[Email Function] RESEND_API_KEY environment variable not found",
      );
      return new Response(
        JSON.stringify({
          error: {
    ...
    if (!apiKey.startsWith("re_")) {
      console.error(
        "[Email Function] Invalid API key format - should start with re_",
      );
      return new Response(
        JSON.stringify({
          error: {
    ...
    const defaultFrom = "TechHub Lesotho <noreply@techhublesotho.com>";
    const emailPayload = {
    ...
    const resendResponse = await fetch("https://api.resend.com/emails", {
      ...
      headers: {
        ...
        "User-Agent": "TechHubLesotho-Shop/1.0.0",
      },
    ...
    console.log(
      `[Email Function] Resend API response received in ${resendDuration}ms`,
    );
    console.log(
      `[Email Function] Resend response status: ${resendResponse.status}`,
    );
    console.log(
      `[Email Function] Resend response headers:`,
      Object.fromEntries(resendResponse.headers.entries()),
    );
    ...
    try {
      ...
      console.log(
        `[Email Function] Resend response body length: ${responseText.length}`,
      );
      data = JSON.parse(responseText);
      console.log("[Email Function] Resend response data:", data);
    } catch (parseError) {
      console.error(
        "[Email Function] Failed to parse Resend response:",
        parseError,
      );
      return new Response(
        JSON.stringify({
    ...
    if (resendResponse.ok) {
      console.log(
        `[Email Function] ✅ Email sent successfully! ID: ${data.id}`,
      );
    } else {
    ...
    const totalDuration = Date.now() - startTime;
    console.error(
      `[Email Function] ❌ Unexpected error after ${totalDuration}ms:`,
      error,
    );
