...
async function fetchProductData(productId, env) {
  try {
    const projectId =
      env.FIREBASE_PROJECT_ID || env.REACT_APP_FIREBASE_PROJECT_ID;
    ...
    console.log(
      `[Meta Middleware] Fetching product ${productId} from Firestore...`,
    );
    ...
    if (!response.ok) {
      console.error(
        `[Meta Middleware] Firestore API error: ${response.status} ${response.statusText}`,
      );
      return null;
    }
    ...
    console.log(
      `[Meta Middleware] Product fetched successfully:`,
      product.name,
    );
    return product;
  } catch (error) {
...
function generateMetaTags(product, url) {
  const metaDescription = product.description
    ...
    : `Buy ${product.name} online at TechHub Lesotho`;

  const title = `${product.name} | TechHub Lesotho - Electronics & Gadgets`;
  ...
  return `
    ...
    <meta property="og:site_name" content="TechHub Lesotho - Technology for You">
    ...
    <meta name="twitter:site" content="@TechHubLesotho">
...
export async function onRequest(context) {
  ...
  if (productPageMatch && isBot(userAgent)) {
    ...
    if (product) {
      ...
      console.log(
        `[Meta Middleware] Meta tags injected for product: ${product.name}`,
      );
      ...
      console.log(
        `[Meta Middleware] Product ${productId} not found, serving default HTML`,
      );
    }
  }
