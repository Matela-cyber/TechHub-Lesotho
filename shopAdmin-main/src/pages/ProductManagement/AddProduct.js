...
const GOODS_CATEGORIES = [
  ...
  {
    value: "Desktops & All-in-Ones",
    label: "Desktops & All-in-Ones",
    icon: "🖥️",
  },
  {
    value: "Servers & Infrastructure",
    label: "Servers & Infrastructure",
    icon: "🗄️",
  },
  {
    value: "Computer Components & Parts",
  ...
  { value: "Printers & Scanners", label: "Printers & Scanners", icon: "🖨️" },
  {
    ...
    label: "Storage Devices (HDD/SSD/NAS)",
    icon: "💾",
  },
  {
  ...
  {
    value: "UPS & Power Solutions",
    label: "UPS & Power Solutions",
    icon: "🔋",
  },
...
const SERVICE_CATEGORIES = [
  {
    ...
    icon: "📦",
    hint: "One-time or subscription software keys",
  },
  {
    value: "Cloud Services & SaaS",
    label: "Cloud Services & SaaS",
    icon: "☁️",
    hint: "Microsoft 365, Google Workspace, CRM, ERP",
  },
  {
    value: "Web Hosting & Domains",
    label: "Web Hosting & Domains",
    icon: "🌐",
    hint: "Shared, VPS, dedicated, domain registration",
  },
  {
    value: "VPS & Dedicated Servers",
    label: "VPS & Dedicated Servers",
    icon: "🗄️",
    hint: "Root access, cPanel, custom specs",
  },
  {
    value: "Managed IT Services",
    label: "Managed IT Services",
    icon: "🛠️",
    hint: "Support contracts, remote management, helpdesk",
  },
  {
    value: "Cybersecurity Services",
    label: "Cybersecurity Services",
    icon: "🔐",
    hint: "Security audits, antivirus subscriptions, SOC",
  },
  {
    value: "Online Courses & Training",
    label: "Online Courses & Training",
    icon: "🎓",
    hint: "Self-paced or instructor-led courses",
  },
  {
    value: "API & Developer Tools",
    label: "API & Developer Tools",
    icon: "⚙️",
    hint: "API access, SDKs, developer subscriptions",
  },
  {
    value: "Data & Analytics Services",
    label: "Data & Analytics Services",
    icon: "📊",
    hint: "Data storage, BI tools, analytics platforms",
  },
  {
    value: "Backup & Disaster Recovery",
    label: "Backup & Disaster Recovery",
    icon: "🔄",
    hint: "Cloud backup, DR plans, replication",
  },
  {
    value: "Telecommunications",
    label: "Telecommunications",
    icon: "📞",
    hint: "VoIP, PBX, internet bundles, SIM plans",
  },
  {
    value: "IT Consulting & Projects",
    label: "IT Consulting & Projects",
    icon: "💡",
    hint: "Project-based or retainer consulting",
  },
];
...
const AddProduct = () => {
  ...
  const checkSlugAvailability = async () => {
    ...
    try {
      const formatted = newProduct.slug
        .toLowerCase()
        .trim()
        .replace(/\s+/g, "-")
        .replace(/[^\w-]/g, "");
  ...
  const handleAddProduct = async () => {
    ...
    try {
      ...
      const base = {
        ...
        mrp: Number(
          newProduct.mrp || newProduct.sellingPrice || newProduct.price,
        ),
      ...
      const extra = isService
        ? {
            ...
            annualPrice: newProduct.annualPrice
              ? Number(newProduct.annualPrice)
              : null,
  ...
  const removeTag = (t) =>
    setNewProduct((p) => ({ ...p, tags: p.tags.filter((x) => x !== t) }));
    ...
    >
      ...
      <Card>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          >
            <div
              className={`p-2 rounded-lg ${!isService ? "bg-blue-100" : "bg-gray-100"}`}
            >
              <Package
                className={`w-6 h-6 ${!isService ? "text-blue-600" : "text-gray-500"}`}
              />
            </div>
            <div>
              <p
                className={`font-bold ${!isService ? "text-blue-700" : "text-gray-700"}`}
              >
                Physical IT Product
              </p>
              <p className="text-xs text-gray-500 mt-0.5">
                Laptop, desktop, server, networking gear, accessories
              </p>
            </div>
          ...
          >
            <div
              className={`p-2 rounded-lg ${isService ? "bg-purple-100" : "bg-gray-100"}`}
            >
              <Cloud
                className={`w-6 h-6 ${isService ? "text-purple-600" : "text-gray-500"}`}
              />
            </div>
            <div>
              <p
                className={`font-bold ${isService ? "text-purple-700" : "text-gray-700"}`}
              >
                Digital Service / Subscription
              </p>
              <p className="text-xs text-gray-500 mt-0.5">
                SaaS, hosting, cloud, courses, licenses, managed IT, API
              </p>
            </div>
      ...
      {activeTab === "basic" && (
        ...
        >
          <Card
            title={isService ? "Service Information" : "Product Information"}
            icon={<Package className={`w-5 h-5 ${accentCls}`} />}
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <Input
                label={isService ? "Service / Plan Name" : "Product Name"}
                placeholder={
                  isService
                    ? "e.g., Business Web Hosting – Starter Plan"
                    : "e.g., Dell Latitude 5540 Laptop"
                }
              ...
              <div>
                ...
                >
                  <option value="">
                    Select {isService ? "Provider" : "Brand"}
                  </option>
                  {brands.map((b) => (
          ...
          {isService && (
            <Card
              title="Delivery & Access Configuration"
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  ...
                  >
                    <option value="instant">
                      ⚡ Instant – automated (API key, link, credentials emailed
                      immediately)
                    </option>
                    <option value="manual-setup">
                      🛠️ Manual Setup – admin configures then delivers
                    </option>
                    <option value="scheduled">
                      📅 Scheduled – on agreed activation date
                    </option>
                  </select>
                </div>
                ...
                <div>
                  ...
                  <select
                    value={newProduct.credentialsDelivery}
                    onChange={(e) =>
                      setNewProduct((p) => ({
                        ...p,
                        credentialsDelivery: e.target.value,
                      }))
                    }
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
                  >
                    ...
                    <option value="dashboard">
                      🖥️ Displayed in customer dashboard
                    </option>
                    <option value="both">Both (email + dashboard)</option>
                    <option value="manual">
                      🤝 Delivered manually by staff
                    </option>
                  </select>
                </div>
                ...
                <div className="col-span-1 md:col-span-2">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Where customers log in or access the service
                  </p>
                </div>
                ...
                <div className="col-span-1 md:col-span-2">
                  ...
                  <textarea
                    value={newProduct.accessInstructions}
                    onChange={(e) =>
                      setNewProduct((p) => ({
                        ...p,
                ...
                <div className="flex items-center gap-3 col-span-1 md:col-span-2">
                  <input
                    type="checkbox"
                    id="apiKey"
                    checked={newProduct.apiKeyProvided}
                    onChange={(e) =>
                      setNewProduct((p) => ({
                        ...p,
                  ...
                  >
                    🔑 This service includes an API key / licence key delivered
                    to the customer
                  </label>
                </div>
              </div>
              ...
              {isTelecom && (
                <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-5 pt-4 border-t border-gray-100">
                  <Input
                    label="Data Quota / Bandwidth"
                    placeholder="e.g., 100 GB/month, Unlimited"
                  ...
                  <Input
                    ...
                    placeholder="e.g., Month-to-month, 12 months, 24 months"
              ...
              {isCourse && (
                <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-5 pt-4 border-t border-gray-100">
                  <Input
                    ...
                    placeholder="e.g., 40 hours, 8 weeks, Self-paced"
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="live-online">
                        🎙️ Live online (scheduled)
                      </option>
                  ...
                  <div className="flex items-center gap-3">
                    <input
                      type="checkbox"
                      id="cert"
                      checked={newProduct.certificateIncluded}
                      onChange={(e) =>
                        setNewProduct((p) => ({
                          ...p,
          ...
          >
            <Alert
              variant="info"
              message="The URL slug is the unique web address for this listing. It cannot be changed after creation."
            />
            <div className="mt-4">
              <label className="block text-sm font-semibold text-gray-700 mb-2">
              ...
              <div className="flex gap-2">
                <div className="flex-1">
                  <div className="flex items-center">
                    <span className="bg-gray-100 px-3 py-2 border border-r-0 rounded-l-lg text-gray-500 text-sm">
                      /product/
                    </span>
                    <input
                      type="text"
                  ...
                  {slugAvailability.checked && (
                    <p
                      className={`text-xs mt-1 ${slugAvailability.available ? "text-green-600" : "text-red-600"}`}
                    >
                      {slugAvailability.available
                        ? "✅ URL is available!"
                        : "❌ URL already taken"}
              ...
              {isService && (
                <div className="col-span-1 md:col-span-2">
                  ...
                  {newProduct.billingCycle === "custom" && (
                    <div className="mt-3">
                      <Input
                        placeholder="e.g., Per project, Per GB, Per API call"
                        value={newProduct.customBillingLabel}
                        onChange={(e) =>
                          setNewProduct((p) => ({
                            ...p,
                            customBillingLabel: e.target.value,
                          }))
                        }
              ...
              {isService ? (
                <>
                  ...
                  {newProduct.billingCycle === "monthly" && (
                    <Input
                      label="Annual Price (M) — discounted yearly rate (optional)"
                      type="number"
                      placeholder="M 0.00"
                  ...
                  <Input
                    label="Compare-at / Original Price (M) — shown crossed out"
                    type="number"
                    placeholder="M 0.00"
                  ...
                  <Input
                    label="One-time Setup / Activation Fee (M) — leave blank if free"
                    type="number"
                    placeholder="M 0.00"
                    value={newProduct.setupFee}
                    onChange={(e) =>
                      setNewProduct((p) => ({ ...p, setupFee: e.target.value }))
                    }
                  />
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        type="number"
                        min="0"
                        value={newProduct.trialPeriodDays}
                        onChange={(e) =>
                          setNewProduct((p) => ({
                            ...p,
                  ...
                  <div>
                    ...
                    >
                      <option value="auto">
                        🔄 Auto-Renewal (charged automatically)
                      </option>
                      <option value="manual">
                        📋 Manual Renewal (customer renews)
                      </option>
                      <option value="none">
                ...
                <>
                  <Input
                    label="MRP — Maximum Retail Price (M)"
                    type="number"
                    placeholder="M 0.00"
                    value={newProduct.mrp}
                    onChange={(e) =>
      ...
      {activeTab === "images" && (
        ...
        >
          <Card
            title={isService ? "Service Logo / Screenshots" : "Product Images"}
              ...
              {["image", "image2", "image3"].map((imgKey, index) => (
                <div key={imgKey}>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setNewProduct((p) => ({ ...p, [imgKey]: e.target.value }))
                    }
      ...
      {activeTab === "details" && (
        ...
        >
          <Card title="Tags" icon={<Tag className="w-5 h-5 text-indigo-600" />}>
            ...
            <div className="flex gap-2">
              <Input
                placeholder={
                  isService
                    ? "e.g., cloud, SaaS, Microsoft, office365"
                    : "e.g., dell, laptop, i7, gaming"
                }
                value={tagInput}
          ...
          <Card
            title={
              isService ? "What is Included (Plan Features)" : "Key Features"
            }
          ...
          <Card
            title={
              isService
                ? "Plan Specifications / Technical Specs"
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-2">
              <Input
                placeholder={
                  isService
                    ? "e.g., Storage, vCPU, RAM, Bandwidth"
            ...
            <div className="space-y-4">
              {!isService && (
                <Input
                  ...
                  placeholder="e.g., USA, China, Germany, Japan"
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  {isService
                    ? "Terms / Compatibility Notes / Fine Print"
      ...
      {activeTab === "warranty" && !isService && (
        ...
        >
              ...
              <div className="flex items-center gap-2 mb-4">
                <input
                  ...
                  onChange={(e) =>
                    setNewProduct((p) => ({
                      ...p,
                      warranty: { ...p.warranty, available: e.target.checked },
                    }))
                  }
                  className="w-4 h-4"
                />
                ...
                >
                  This product includes a manufacturer warranty
                </label>
              </div>
                ...
                <div className="pl-6 space-y-4">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows="3"
                      placeholder="What does the warranty cover? On-site, carry-in, parts only, labour included..."
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                    />
                  </div>
                </div>
            ...
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <label
                  ...
                  className="font-semibold text-gray-700 cursor-pointer text-sm"
                >
                  This product includes a satisfaction guarantee
                </label>
              </div>
                ...
                <div className="pl-6 space-y-4">
                  <Input
                    ...
                    onChange={(e) =>
                      setNewProduct((p) => ({
                        ...p,
                        guarantee: { ...p.guarantee, period: e.target.value },
                      }))
                    }
                  />
                  <div>
                    ...
                    <textarea
                      ...
                      rows="3"
                      placeholder="Money-back guarantee, replacement policy, exchange conditions..."
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                    />
                  </div>
                </div>
          ...
          <Card
            title="Import & Shipping Information"
              ...
              <div className="pl-6 space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows="2"
                    placeholder="e.g., Allow 2–4 weeks for delivery from overseas supplier"
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
                  />
                </div>
              </div>
      ...
      {activeTab === "service" && isService && (
        ...
        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <Input
                ...
                onChange={(e) =>
                  setNewProduct((p) => ({
                    ...p,
                    sla: { ...p.sla, uptime: e.target.value },
                  }))
                }
              />
              <Input
                label="Support Response Time"
                placeholder="e.g., 4 hours, 24 hours, Next business day"
                value={newProduct.sla.responseTime}
                onChange={(e) =>
                  setNewProduct((p) => ({
                    ...p,
                    sla: { ...p.sla, responseTime: e.target.value },
                  }))
                }
              />
              <div>
              ...
              <div className="col-span-1 md:col-span-2">
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  Full SLA Description
                </label>
                <textarea
                  value={newProduct.sla.details}
                  onChange={(e) =>
                    setNewProduct((p) => ({
                      ...p,
                      sla: { ...p.sla, details: e.target.value },
                    }))
                  }
                  rows="3"
          ...
          <Card
            title="Users, Seats & Compliance"
            icon={<Users className="w-5 h-5 text-indigo-600" />}
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  Max Users / Seats included
                </label>
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    ...
                    value={newProduct.maxUsers}
                    onChange={(e) =>
                      setNewProduct((p) => ({ ...p, maxUsers: e.target.value }))
                    }
                    placeholder="0 = unlimited"
